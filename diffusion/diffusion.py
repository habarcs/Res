...
class Diffusion:
    ...
    def forward_process(
        self, lq: Tensor, hq: Tensor, t: Tensor, device: torch.device
    ) -> Tensor:
        """
        ...
        lq and hq and t should already be on right device
        """
    ...
    def reverse_process(
        ...
        lq and f_theta should already be on right device
        """
        assert lq.dim() == 4
        ...
        for t in range(self.T, 1, -1):  # T, T-1, ... 3, 2
            ...
            x_0 = f_theta(x_t, lq, torch.full(t_dim, t, device=device))
        ...
        return f_theta(x_t, lq, torch.full(t_dim, 1, device=device)), progress
    ...
    def __shift_t(self, t: Tensor, device: torch.device) -> Tensor:
        """
        ...
        return (
            torch.zeros(t.size(), dtype=torch.float)
            .map_(t.cpu().float(), lambda _, x: self.shifting_seq(int(x)))
            .to(device)
        )
